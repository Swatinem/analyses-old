...
// TODO: avoid creating cfg nodes for Expressions
// TODO: use a ast->cfg map to
//         avoid adding and deleting properties on ast nodes
//         make reversing the graph cleaner

/**
 * Returns [entry, exit] `FlowNode`s for the passed in AST
 */
var ControlFlowGraph = module.exports = function (astNode) {
	var parentStack = [];
	...
	function reverse(cfgNode) {
		// do not run into an infinite loop
		if (cfgNode.next)
			return;
		cfgNode.next = [];
		['normal', 'true', 'false', 'exception'].forEach(function (type) {
			if (!cfgNode[type])
				return;
			if (!~cfgNode.next.indexOf(cfgNode[type]))
				cfgNode.next.push(cfgNode[type]);
			if (!~cfgNode[type].prev.indexOf(cfgNode))
				cfgNode[type].prev.push(cfgNode);
			reverse(cfgNode[type]);
		});
	}
	...
	function getEntry(astNode) {
		switch (astNode.type) {
			...
			case 'FunctionExpression':
				if (astNode.cfg.parent) // the function is included in another AST
					return getSuccessor(astNode);
				else // the function itself is the AST
					return getEntry(astNode.body);
			case 'IfStatement':
	...
	function createNodes(astNode) {
		walker(astNode, {
			default: function () {
			// do not create cfg nodes for expressions
			// TODO: turn this into a whitelist of Statements
			if (~['FunctionExpression'].indexOf(this.type))
				return;
			var parent = parentStack.length ? parentStack[parentStack.length - 1] : undefined;
			createNode(this, parent);
			// do not recurse for functions declarations
			if (~['FunctionDeclaration'].indexOf(this.type))
				return;

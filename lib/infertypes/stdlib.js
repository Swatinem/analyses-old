...
exports.stdlib.Object[0].set('create', FunctionValue(function (thisVal, argVals) {
	var val = new Value({
		typeof: 'object',
		proto: argVals[0][0] // needs to be a single value
	});
	return new Reference(val);
}));
exports.stdlib.Object[0].set('keys', FunctionValue(function (thisVal, argVals) {
	var arr = newArray()[0];
	var props = [];
	argVals[0].forEach(function (obj) {
	...
	props.forEach(function (prop, index) {
		arr.set(index, newString(prop));
	});
	arr.set('length', newNumber(props.length));
	if (argVals[0].length > 1)
		arr.getGeneric('number'); // force merging of number props
	return new Reference(arr);
}));
...
// calls argVals[0] with the array elements, returning the accumulated retVal
function callElements(thisVal, argVals) {
	// collect all the possible types:
	var contents = new Reference();
	thisVal.forEach(function (thisVal) {
		contents.assign(thisVal.getGeneric('number'));
	});
	// call the callback function once with this accumulated type
	var retVal = {retVal: new Reference(), throwVal: new Reference()};
	argVals[0].forEach(function (fun) {
		// FIXME: this needs to interact correctly with the global object `this`
		var newThis = argVals.length > 1 ? argVals[1] : new Reference();
		var functionReturn = fun.evaluateCall(newThis, [contents]);
		retVal.retVal.assign(functionReturn.retVal);
		retVal.throwVal.assign(functionReturn.throwVal);
	});
	return retVal;
}
ArrayProto.set('filter', FunctionValue(function (thisVal, argVals) {
	callElements(thisVal, argVals);
	return thisVal;
}));
ArrayProto.set('some', FunctionValue(function (thisVal, argVals) {
	callElements(thisVal, argVals);
	return newBoolean();
}));
ArrayProto.set('every', FunctionValue(function (thisVal, argVals) {
	callElements(thisVal, argVals);
	return newBoolean();
}));
ArrayProto.set('map', FunctionValue(function (thisVal, argVals) {
	var arr = newArray();
	var retVal = callElements(thisVal, argVals);
	// and create a generic Array with the result
	arr[0].setGeneric(retVal.retVal, 'number');
	return arr;
}));
ArrayProto.set('forEach', FunctionValue(callElements));

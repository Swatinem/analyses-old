...
Completer.prototype.splice = function Completer_splice(offset, remove, add) {
	...
	// FIXME: without an incremental/updating parser, we need to better
	// batch the parsing and do it JIT
...
Completer.prototype.complete = function Completer_complete(offset) {
	...
	try {
		...
		results = filtered.map(function (res) {
			var o = res.original;
			o.markup = res.string;
			o.score = res.score / (1 + o.depth);
			o.type = 'value'; // TODO: or `function`, but hardcoded for now
			o.info = ''; // TODO: markup to show the value/function signature
			delete o.depth;
			return o;
		});
